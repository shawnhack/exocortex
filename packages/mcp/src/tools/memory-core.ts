...
export function registerMemoryCoreTools(ctx: ToolRegistrationContext): void {
  ...
  server.tool(
    ...
    "Store a new memory. Set provider/model_id/agent for attribution, namespace for project scope, deduplicate: true for facts that may already exist.",
    {
      ...
      model_id: z.string().optional().describe("Canonical model identifier — always set this (e.g. 'claude-opus-4-7', 'gpt-5-codex')"),
      model_name: z.string().optional().describe("Human-readable model name — always set this (e.g. 'Claude Opus 4.7', 'GPT-5.3-Codex')"),
      ...
      metadata: z.record(z.string(), z.any()).optional().describe("Arbitrary JSON metadata (e.g. { model: 'claude-opus-4-7' })"),
