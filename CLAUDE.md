...
- Consolidation uses basic (non-LLM) summary generation by default. Optional `LLMSummarizer` interface allows passing a cheap model (Haiku-tier) to `autoConsolidate()` for higher-quality summaries. Consolidation includes content hash dedup, provenance chain tracking, and SAVEPOINT-based nesting for transaction safety
