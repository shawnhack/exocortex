...
- Goal autonomy: metadata keys `mode` ("monitor"|"autonomous"), `approved_tools` (string[]), `max_actions_per_cycle` (number), `strategy` (string). Autonomous goals are worked by Cortex's goal-worker sentinel job
- Goals REST endpoint: `GET /api/goals?status=active` — used by Cortex for pre-flight checks before running the goal-worker LLM call
...
- Consolidation always uses basic (non-LLM) summary generation. LLM-powered synthesis is handled externally by Cortex's sentinel gardening job, avoiding direct API costs in Exocortex
